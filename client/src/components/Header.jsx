...
export default function Header() {
  ...
  return (
    <Navbar className='relative border-b-2 py-2 px-2 w-full flex flex-col items-center justify-between'>
      ...
      <Button className='w-12 h-10 items-center justify-center rounded-full md:hidden' color='gray'>
      ...
      <div className='flex gap-2 items-center md:hidden'>
        ...
        <Link to='/sign-in'>
          <Button className='px-2 bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500' outline>
            Sign In
          </Button>
        </Link>
      ...
      <nav className={`absolute top-full left-0 w-full bg-white md:hidden z-50 ${menuOpen ? 'block' : 'hidden'}`}>
        <ul className='flex flex-col gap-4 p-4'>
          <li>
            <Link 
              to='/' 
              onClick={() => setMenuOpen(false)}
              className={`hover:text-indigo-500 ${isActive('/') ? 'text-indigo-600 font-bold' : ''}`}
            >
              Home
            </Link>
          </li>
          <li>
            <Link 
              to='/about' 
              onClick={() => setMenuOpen(false)}
              className={`hover:text-indigo-500 ${isActive('/about') ? 'text-indigo-600 font-bold' : ''}`}
            >
              About
            </Link>
          </li>
          <li>
            <Link 
              to='/project' 
              onClick={() => setMenuOpen(false)}
              className={`hover:text-indigo-500 ${isActive('/project') ? 'text-indigo-600 font-bold' : ''}`}
            >
              Projects
            </Link>
          </li>
